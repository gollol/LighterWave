from .utils import str_float, LoggingBlock
...
class XMLNode:
    ...
    def add_child(self, child, name=None):
        if child is None:
            LoggingBlock.error(f"Tried adding None child with name '{name}' to '{self.name}' XML node")
    ...
    def add_children(self, children):
        if None in children:
            LoggingBlock.error(f"Tried adding None child to '{self.name}' XML node")
