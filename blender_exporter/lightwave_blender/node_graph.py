        ...
        for input in node.inputs.values():
            if input.is_linked:
                if len(input.links) != 1:
                    node_graph.export_ctx.warn("Multi-input links are not supported!")
                    continue
...
class RMNodeGraph(object):
    def __init__(self, export_ctx: ExportContext, name: str, node_tree: bpy.types.NodeTree):
        self.name = name
    ...
    def delete_node(self, node_name):
        ...
        for (other_name, node) in self.nodes.items():
            for (input_name, link) in node.links.items():
                if link[0] == node_name:
                    self.export_ctx.warn(f"cannot remove node '{node_name}' as '{other_name}'.'{input_name}' still relies on output '{link[1]}'!")
                    return
    ...
    def remove_muted_nodes(self):
        for (node_name, node) in list(self.nodes.items()):
            ...
            self.export_ctx.warn("Node muting has not been tested!")
    ...
    def inline_node_groups_recursively(self, max_depth=8):
        if max_depth == 0:
            self.export_ctx.warn("Maximum depth reached while inlining node group")
            return
        ...
        for (node_name, node) in list(self.nodes.items()):
            ...
            sub_graph = RMNodeGraph(self.export_ctx, self.name, node.bl_node.node_tree)
