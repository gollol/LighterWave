...
/**
 * @brief Visualizes the rays generated by a camera by intersecting them with a
 * planar grid, and encoding the ray's direction as color. The grid is an
 * infinite plane at z=+1 with a grid frequency specified by the user.
 *
...
public:
    ...
    Color Li(const Ray &ray, Sampler &rng, Context &cont) override {
        ...
        if (m_showGrid) {
            ...
            // remap the direction from [-1,+1]^3 to [0,+1]^3 so that colors
            // channels are never negative
            d = (d + Vector(1)) / 2;
        }
        return Color(d);
    }
...
// this informs lightwave to use our class CameraIntegrator whenever a
// <integrator type="camera" /> is found in a scene file
