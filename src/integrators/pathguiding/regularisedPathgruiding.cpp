...
class RegularisedPathGuider : public SamplingIntegrator {
    ...
    float m_dTreeThreshold = 0.1f; // subdivide DTree leaf if the power of the
                                   // light is more than this threshold
    ...
    Color regularisedNextEventEstimation(
        const Intersection &its, const float time, Sampler &rng,
        float regularisedRoughness, std::vector<float> &attenuation,
        const DTreeWrapper *dTree, float bsdfSamplingFraction,
        Vector dTreeVoxelSize, Context &cont) const {
        PROFILE("RNEE")
    ...
    BsdfSample sampleMaterial(Sampler &rng, const DTreeWrapper *dTree,
                              ...
                              float &dTreePdf, float bsdfSamplingFraction,
                              float roughness, Context &cont) {
        if (!its.instance || !its.instance->bsdf()) {
        ...
        if (!dTree || !m_isBuilt ||
            its.instance->bsdf()->getRoughness(its.uv, cont) <=
                m_roughnessThreshold) {
            result   = its.sampleBsdf(rng, cont, roughness);
            woPdf    = result.pdf;
            dTreePdf = 0;
            return result;
        }
        ...
        if (sample < bsdfSamplingFraction) {
            ...
            BsdfSample bsdfSamp = its.sampleBsdf(rng, cont, roughness);
            ...
            if (std::isinf(bsdfSamp.pdf)) {
                ...
                /*if (!bsdfSamp.weight.isValid()) {
                    logger(EInfo,
                           " bsdf glas und so : bsdfPdf: %f, dTreePdf: %f, "
                           "woPdf: %f",
                           bsdfPdf,
                           dTreePdf,
                           woPdf);
            ...
            BsdfEval bsdfEval = its.evaluateBsdf(wi, cont, roughness);
        ...
        /*if (!result.weight.isValid()) {
            logger(EInfo,
                   " end of fnkt : bsdfPdf: %f, dTreePdf: %f, woPdf: %f",
                   bsdfPdf,
                   dTreePdf,
                   woPdf);
        }*/
    ...
    void splatLightPoint(DTreeWrapper *dTree, float woPdf, float dTreePdf,
                         float bsdfPdf, Color bsdfValue, Color radiance,
                         Ray ray, Vector dTreeVoxelSize, Sampler &rng) {
    ...
    Color recursiveLi(const Ray &ray, Sampler &rng, int depth, float prevWoPdf,
                      ...
                      std::vector<float> &attenuationFactors, Color throughput,
                      Context &cont) {
        auto L = Color(0);
        ...
        // Direct illumination
        auto E = its.evaluateEmission(cont);
        if (E && depth == 1) {
            L += E.value;
        }
        ...
        for (int sampleIndex = 0; sampleIndex < numSamples; ++sampleIndex) {
            ...
            L += regularisedNextEventEstimation(its,
                                                ray.time,
                                                rng,
            ...
            Ray regularisedRay = Ray(ray);
            if (regularisedMaterialSample) {
            ...
            if (dTree && 1 / regularizedWoPdf > 0 &&
                regularisedMaterialSample) {
                splatLightPoint(dTree,
                                regularizedWoPdf,
                                regularizedDTreePdf,
                                regularisedMaterialSample.pdf,
                                regularisedMaterialSample.weight /
                                    regularizedWoPdf,
                                regularisedPathLight,
                                regularisedRay,
                                dTreeVoxelSize,
...
public:
    RegularisedPathGuider(const Properties &properties)
        : SamplingIntegrator(properties) {
        m_maxDepth = properties.get<int>("depth", 2);
        // NEE replaced by m_neeType
...
REGISTER_INTEGRATOR(RegularisedPathGuider, "RegularisedPathGuider")
