...
/**
 * @brief Tests whether the output of an integrator matches a given reference
 * image.
 *
 * Paired with different integrators, this can be used to test nearly all
 * aspects of your renderer: For example, with a normal integrator (which
 * returns the normal vector at the first intersection), this test can verify
 * that your intersection functions report the correct normal.
 *
 * @note Internally, this computes the mean absolute error (MAE) of the image
...
class CompareImage : public Test {
    ...
    float m_thresholdME;
    /// @brief Whether to report an error if any color channels in the output
    /// image are negative.
...
public:
    ...
    void execute() override {
        ...
        if (std::getenv("reference")) {
            ...
            Streaming stream{ *reference, false };
            stream.update();
    ...
    std::string toString() const override { return "CompareImage[]"; }
    ...
    void compare(const Image &image, const Image &reference) const {
        ...
        for (auto pixel : image.bounds()) {
            ...
            for (int channel = 0; channel < r.NumComponents; channel++) {
                if (std::isnan(i[channel]))
                    lightwave_throw("nan encountered at pixel %d,%d", pixel.x(),
                                    pixel.y());
                if (std::isinf(i[channel]))
                    lightwave_throw("infinity encountered at pixel %d,%d",
                                    pixel.x(), pixel.y());
                if (!m_allowNegative && i[channel] < 0)
                    lightwave_throw("negative value encountered at pixel %d,%d",
                                    pixel.x(), pixel.y());
            }
        ...
        if (absError > m_thresholdMAE)
            lightwave_throw("absolute error threshold exceeded (%.3g > %.3g)",
                            absError, m_thresholdMAE);
        if (abs(float(error)) > m_thresholdME)
            lightwave_throw("error threshold exceeded (%.3g > %.3g)",
                            abs(float(error)), m_thresholdME);
    }
};
