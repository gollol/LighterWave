...
static void readPlyContent(std::istream &stream, const Header &header,
    ...
    for (int i = 0; i < header.VertexCount; ++i) {
        ...
        // TODO: find a way to add vector type conversions with #ifdef's instead
        // of encode and decode functions
        // vertex.normal = encode_spherical(Vector(nx, ny, nz).normalized());
        // vertex.normal = encode_oct(Vector(nx, ny, nz).normalized());
        vertex.normal =
            bob::CompressedNormal(Vector(nx, ny, nz).normalized().data());
        vertex.uv = { u, v };
    ...
    if (indicesIndex != indices.size())
        lightwave_throw("too few faces (%d found, %d needed)",
                        indicesIndex,
                        indices.size());

    if (!header.hasUVs()) {
        Bounds bbox;
        for (const bob::Vertex &v : vertices)
            bbox.extend({ v.position.data() });

        for (size_t i = 0; i < vertices.size(); ++i) {
            auto &v        = vertices.at(i);
            const Vector d = bbox.diagonal();
            const Vector t = Point(v.position.data()) - bbox.min();
            ...
            if (d.y() > Epsilon)
                p.y() = t.y() / d.y();
            v.uv = { p.x(), p.y() }; // Drop the z coordinate
        }
    }
}
