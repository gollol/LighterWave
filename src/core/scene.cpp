...
Scene::Scene(const Properties &properties) {
    ...
    m_background = properties.getChildren<BackgroundLight>();
...
Intersection Scene::intersect(const Ray &ray, Sampler &rng, Context &cont) const {
    PROFILE("Intersect")
    ...
    cont.its = &its;
    cont.negRayDirection = -ray.direction;
    cont.backFacing = !Frame::sameHemisphere(-ray.direction, its.geometryNormal);

    its.lightProbability = m_lightSampling->probability(its.light());
    return its;
}

bool Scene::intersect(const Ray &ray, float tMax, Sampler &rng, Context &cont) const {
    PROFILE("Shadow ray")
    Context copy = Context(cont);
    Intersection its(-ray.direction, tMax * (1 - Epsilon));
    return m_shape->intersect(ray, its, rng, copy);
}
