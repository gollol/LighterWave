...
struct LightSample {
    ...
    /// @brief Indicates light sampling has failed (e.g., the scene has no explicit lights).
...
class Scene : public Object {
    ...
    /// the scene.
    std::vector<ref<BackgroundLight>> m_background;
    
    class LightSampling;
    /// @brief Contains all necessary data structures to randomly pick light sources.
...
public:
    ...
    Intersection intersect(const Ray &ray, Sampler &rng, Context &cont) const;
    /// @brief Reports whether any intersection up to a given maximal distance
    /// exists (used for testing visibility of light sources).
    bool intersect(const Ray &ray, float tMax, Sampler &rng, Context &cont) const;
