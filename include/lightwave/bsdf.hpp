...
public:
    virtual Color albedo(const Point2 &uv, const Context &cont) const = 0;
    /**
    ...
    virtual BsdfEval evaluate(const Point2 &uv, const Vector &wo,
                              const Vector &wi, const Context &cont,
     ...
     */
    virtual BsdfSample sample(const Point2 &uv, const Vector &wo, Sampler &rng,
                              const Context &cont,
                              float roughness = 0) const = 0;

    virtual float getRoughness(const Point2 &uv, const Context &cont) const = 0;
};
