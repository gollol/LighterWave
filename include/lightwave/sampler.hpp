...
namespace lightwave
{
    ...
    /**
     * @brief Samplers are pseudo-random number generators that steer sampling
     * decisions. While all samplers are expected to produce uniformly distributed
     * random numbers, they can still differ in how they correlate samples, e.g.,
     * some samplers try to cover the space of random numbers more evenly by by
     * avoiding placing samples close to previous samples.
     */
    ...
    public:
        ...
        virtual Point2 next2D() { return {next(), next()}; }
        /// @brief Generates a random point in the unit cube [0,1)^3.
        virtual Vector next3D() { return {next(), next(), next()}; }
        ...
        /**
         * @brief Initiates a random number sequence characterized for the given
         * pixel and sample per pixel index.
         * @note When identical samplers are given the same seed, they are expected
         * to produce the same sequence of random numbers. For different seeds, they
         * are expected to give different random sequences.
         */
