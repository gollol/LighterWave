...
public:
    ...
    template <typename T>
    std::enable_if_t<std::is_base_of_v<Object, T>, void>
    set(const std::string &name, const ref<T> &object, int loc) {
        if (has(name)) {
            lightwave_throw("property \"%s\" redefined %d", name, loc);
        }
    ...
    get(const std::string &name) const {
        ...
        return std::visit(
            overloaded{
                [&](auto &&arg) -> T {
                    lightwave_throw(
                        "property \"%s\" expects type %s, but found %s",
                        name,
...
#pragma warning(pop)
    ...
    /// @brief Gets the value of an attribute, with a fallback to null if the
    // attribute is not present.
    template <typename T>
    std::enable_if_t<std::is_base_of_v<Object, T>, ref<T>>
    ...
    /// @brief Lists unused attributes and children as warning on the console.
    ~Properties() {
        // do not confuse users with spurious unused property warnings (they
        // might have been used if the code was not interrupted by an exception)
        if (std::uncaught_exceptions())
            return;

        for (auto &child : m_unqueriedChildren) {
            logger(EWarn,
                   "a child node was specified, but never queried: %s",
                   child);
        }

        for (auto &property : m_unqueriedAttributes) {
            logger(EWarn,
                   "attribute \"%s\" was specified, but never queried",
                   property);
        }
    }
};
