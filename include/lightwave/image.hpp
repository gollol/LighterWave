...
class Image final : public Object {
    ...
    Point2i pixelFromNormalized(const Point2 &normalized) const {
        return { std::clamp(int(normalized.x() * m_resolution.x()),
                            0,
                            m_resolution.x() - 1),
                 std::clamp(int(normalized.y() * m_resolution.y()),
                            0,
                            m_resolution.y() - 1) };
    }
...
public:
    ...
    Image(const Properties &properties) {
        if (properties.has("filename")) {
            ...
            // Image with constant color from XML node
            Color color = properties.get<Color>("color");
            initializeConstantColor(Point2i(2, 2), color);
        } else {
    ...
    /// @brief Changes the resolution and sets all pxiels to a fixed color.
    void initializeConstantColor(const Point2i &resolution,
                                 const Color &color) {
        m_resolution = resolution;
        m_data.resize(resolution.x() * resolution.y());
        std::fill(m_data.begin(), m_data.end(), color);
    ...
    void save() const {
        std::filesystem::path o = m_basePath / (id() + ".exr");
        logger(EInfo, "\n\n\n\nsaving image %s\n\n\n\n", o);
     ...
     */
    const Color &get(const Point2 &normalized) const {
     ...
     */
    Color &get(const Point2 &normalized) {
        return (*this)(pixelFromNormalized(normalized));
    }
