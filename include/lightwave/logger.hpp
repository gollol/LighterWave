...
// Uncomment this to enable debug logging of pixel coordinates and sample
//#define DEBUG_PIXEL_LOGGER

#ifdef DEBUG_PIXEL_LOGGER
typedef struct t_debugPixelState {
    bool active = false;
    int x = 0;
    int y = 0;
    int sample = 0;
...
extern thread_local t_debugPixelState debugPixelState;
#endif
    ...
    void operator()(LogLevel level, const char *fmt, const Args &...args) {
        ...
        switch (level) {
            case EDebug:
                ...
                #ifdef DEBUG_PIXEL_LOGGER
                if (debugPixelState.active) {
                    std::cout << tfm::format("[(%d,%d) #%d] ", debugPixelState.x, debugPixelState.y, debugPixelState.sample);
                }
                #endif
                break;
            case EInfo:
