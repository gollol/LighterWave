...
/// @brief Returns the floored integer value of @p x and stores the remaining
/// decial in @p i
...
/**
 * @brief Safe arcus sine function, which clamps the input to -1 to +1.
 * @note Use this when floating point errors need to be avoided.
 */
inline float safe_asin(float v) { return std::asin(clamp(v, -1.f, +1.f)); }
/**
 * @brief Safe powf function, based on
 * https://github.com/uzairakbar/blender/blob/master/blender/2.79/scripts/addons/cycles/source/util/util_math.h.
 * @note Use this when floating point errors need to be avoided.
 */
...
inline bool solveQuadratic(const float &b, const float &c, float &x0,
                           float &x1) {
    // assumes that a=1
    float discr = b * b - 4 * c;
    if (discr < 0)
...
public:
    ...
    friend TPoint operator*(const Type &a, const TPoint &b) {
        TPoint BUILD1(a * b[i])
    }
    friend TPoint operator*(const TPoint &a, Type b) { TPoint BUILD1(a[i] * b) }
    friend TPoint operator+(const TPoint &a, const TPoint &b) {
        TPoint BUILD1(a[i] + b[i])
    }
    friend TPoint operator-(const TPoint &a) { TPoint BUILD1(-a[i]) }
};
...
public:
    ...
    /// @brief Expand the bounding box by a given delta in all directions.
...
/// @brief A two-dimensional axis-aligned bounding box with floating point
/// components.
using Bounds2 = TBounds<float, 2>;
...
/// @brief A 3x4 matrix with floating point components (used to transform
/// hair coordinates, optional).
...
/// @brief A four-dimensional array used for interpolation of 4 Colors.
using Colors4 = std::array<Color, 4>;
/// @brief A four-dimensional array used for interpolation of 4 Points.
using Points4 = std::array<Point, 4>;
/// @brief A four-dimensional floats array used for interpolation in 1
/// dimension.
using Floats4 = std::array<TPoint<float, 1>, 4>;
inline Floats4 toFloats4(const std::array<float, 4> a) {
    std::array<TPoint<float, 1>, 4> res = { TPoint<float, 1>{ a[0] },
                                            TPoint<float, 1>{ a[1] },
                                            TPoint<float, 1>{ a[2] },
                                            TPoint<float, 1>{ a[3] } };
    return res;
...
struct Ray {
    ...
    Vector direction;
    /// @brief The time at which the ray was created (Used for motion blur).
    float time = 0;
    ...
    Ray(Point origin, Vector direction, float time, int depth)
        : origin(origin), direction(direction), time(time), depth(depth) {}

    /// @brief Constructor when depth and time are irrelevant.
    Ray(Point origin, Vector direction)
        : origin(origin), direction(direction) {};

    /// @brief Constructor for empty ray.
    Ray() {}
    ...
    Ray normalized() const {
        return Ray(origin, direction.normalized(), time, depth);
    }
};
...
struct Vertex {
    ...
    Vertex(const Point &p, const Vector2 &u, const Vector &n)
        : position(p), uv(u), normal(n) {}

    Vertex(const bob::Vertex &v)
        : position(v.position.data()), uv(v.uv.data()),
          normal(v.normal.decompress().data()) {}

    /// @brief Barycentric interpolation of vertices ([0,0] returns a,
    /// [1,0] returns b, and [0,1] returns c).
    static Vertex interpolate(const Vector2 &bary, const Vertex &a,
                              const Vertex &b, const Vertex &c) {
        return {
            interpolateBarycentric(bary, a.position, b.position, c.position),
            interpolateBarycentric(bary, a.uv, b.uv, c.uv),
...
/// @brief Some further information needed in bsdfs / textures.
struct Context {
    Intersection *its      = nullptr;
    Sampler *rng           = nullptr;
    Scene *scene           = nullptr;
    Vector wo              = Vector(0);
    Vector negRayDirection = Vector(0);
    int curvesIndex        = -1;
...
struct Intersection : public SurfaceEvent {
    ...
    /**
     * @brief The probability of the choosing, this particular background
     */
    float backgroundProbability = 0.0f;
    ...
    bool testAlpha(const Point2 &uv, Sampler &rng, Context &cont) const;
    ...
    EmissionEval evaluateEmission(Context &cont) const;
    float directPdf() const;
    /// @brief Samples the Bsdf of the underlying surface.
    BsdfSample sampleBsdf(Sampler &rng, Context &cont,
                          float roughness = 0) const;
    BsdfEval evaluateBsdf(const Vector &wi, Context &cont,
                          float roughness = 0) const;
    float vertexRoughness(const Context &cont) const;
...
/// @brief Asserts that a given vector is normalized, and aborts if that is not
/// the case.
