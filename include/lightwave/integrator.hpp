...
/**
 * @brief Integrators are rendering algorithms that take a scene and produce an
 * image from them (e.g., using path tracing). The term integrator refers to the
 * key challenge of simulating light transport, namely solving the reflected
 * radiance integral. Integrators can also be used to visualize quantities other
 * than radiance for debugging purposes, for example, visualizing the normals of
 * surfaces that were intersected.
 */
...
/**
 * @brief A sampling integrator uses random numbers to solve the integration
 * problem, e.g., by using Monte Carlo integration.
 */
...
public:
    SamplingIntegrator(const Properties &properties) : Integrator(properties) {
    ...
    /// @brief Computes all pixels of the image by constructing camera rays for
    /// them and invoking the @ref Li method.
    void execute() override;

    /**
     * @brief Returns (an estimate of) the incident radiance for a given ray.
     * By default, the integrator will take care of looping over all pixels,
     * constructing camera rays for each of them, and then invoking this method
     * to determine the pixel values. If you need to customize this process,
     * override the
     * @ref execute function of the integrator.
     */
    virtual Color Li(const Ray &ray, Sampler &rng, Context &cont) = 0;
};
