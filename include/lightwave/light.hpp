...
struct DirectLightSample {
    ...
    Color weight;
    /// @brief The probability density of having sampled direction wi, in solid
    /// angle units.
    float pdf;
...
protected:
    /**
     * @brief How likely this light will be selected when picking a random light source.
     * A value of zero indicates that this light should never be sampled.
     * The probability of sampling this light will be its weight divided by the total weight
     * of all light sources in the scene.
     */
...
public:
    BackgroundLight(const Properties &properties)
    : Light(properties) {}
